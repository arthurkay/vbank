...
**v1.5 (2026-08-26) - desktop builds with native design systems (§37)**
- One domain layer, four presentations. `AppPlatform.shell` (lib/core/app_platform.dart) picks the shell and `VBankRoot` in main.dart mounts it: shadcn_flutter on Android/iOS, **Yaru on Linux**, **macos_ui on macOS**, **Fluent on Windows**. Services, providers, crypto, storage and sync are untouched and shared.
- Platform-neutral list logic moved to `lib/core/presentation/list_filters.dart` (`ListFilter`, haystacks, formatters) so all four shells drive the same filters with their own controls: shadcn chips, Yaru `YaruChoiceChipBar`, macOS `MacosSegmentedControl`, Fluent `ComboBox`.
- Start-up work is shared in `lib/core/app_bootstrap.dart` (identity load → sync identity → node start → notifications → deep links). The first Linux build hung on its spinner precisely because the desktop shells lacked it; all four now wrap themselves in `AppBootstrap`.
- Linux (`lib/desktop/linux/`): `YaruMasterDetailPage` destinations pane, `YaruTabBar` group sections, `YaruSection`/`YaruListTile` content, `YaruSearchField` + chip bar, `YaruDialogTitleBar` dialogs, snack-bar feedback.
- macOS (`lib/desktop/macos/`): `MacosWindow` + source-list `Sidebar`, per-page `ToolBar`, `MacosTabView` sections, grouped rows, `MacosSheet` forms, `MacosAlertDialog` confirmations, `MacosDatePicker`/`MacosTimePicker`.
- Windows (`lib/desktop/windows/`): `NavigationView` + `NavigationPane` (Sync carries an `InfoBadge` on error), `ScaffoldPage` + `PageHeader` + `CommandBar`, `TabView` sections, `Card`/`ListTile` groups, `ContentDialog` forms, `InfoBar` feedback, Fluent `DatePicker`/`TimePicker`.
- Encrypted storage on desktop: `sqflite_sqlcipher` has no Linux/Windows plugin, so those platforms open the same file through `sqflite_common_ffi` against the **SQLite3MultipleCiphers** build of `package:sqlite3` (`hooks.user_defines.sqlite3.source: sqlite3mc` in pubspec.yaml) in SQLCipher-compatible mode (`PRAGMA cipher = 'sqlcipher'` then `PRAGMA key`). Verified on Linux: the file header is ciphertext and plain `sqlite3` reports "file is not a database". Android/iOS/macOS keep the native SQLCipher plugin.
- `DeviceSecret` now falls back to a `0600` file when no Secret Service is reachable, and bounds the keyring call (3 s) — without that, Linux start-up blocked on D-Bus forever.
- Platform gaps, handled explicitly: no camera QR scanning on Linux/Windows (`AppPlatform.canScanQr`) — those shells join by pasted invite link; no Windows notification backend in flutter_local_notifications 18 (`AppPlatform.canNotify`), so `main()` skips notification init there and the Settings page says so. `fluent_ui` is pinned to 4.15.1 because 4.16.x needs a newer Flutter than 3.41.6.
- Shutdown hardening: `IpfsService.dispose` stops the node before closing its stream, and both it and `SyncManager` guard every `add` against a closed controller (a desktop teardown surfaced "Cannot add new events after calling close").
- Verified: `integration_test/linux_shell_test.dart` drives the real Linux app (onboarding → identity → shell → create group with PBKDF2 → sections → encrypted-DB assertion) with `flutter test integration_test/linux_shell_test.dart -d linux`. macOS and Windows shells are analyzer-clean but unbuilt — no Apple or Windows hardware in this environment.
...
**v1.7 (2026-08-27) — Restyle, money at a glance, loans with their repayments**
- **Cleaner mobile UI.** Lists are flat rows separated by hairlines (`ListRow`), grouped facts live in `Panel`s with right-aligned receipt lines (`InfoRow`), section titles are small tracked capitals, radius 16. The one strong surface per page is the inverted "ticket" `HeroCard` (near-black on light, white on dark, dashed perforation with notches) — the same motif as the app icon and the inspiration screens — with `Metric`s and a `ProgressBar`.
- **Where the money is.** `GroupFund.compute(transactions, loans)` (`lib/models/loan_progress.dart`) splits the fund into cash **available to lend** (contributions + repayments + penalties + fees − loans − withdrawals), principal **out on loan** (approved-but-unpaid loans count as committed) and interest still expected; `groupFundProvider`. Shown on the group overview hero, in reports and in all three desktop shells.
- **Loans always show their repayments.** `LoanProgress` pairs a loan with its confirmed repayment transactions (by `loanId`): borrowed / repaid / still owed, fraction, principal outstanding (repayments count against principal first). `groupLoanProgressProvider` / `loanProgressProvider`. Used by `LoanTile` (lists and tabs), `LoanProgressView` + `RepaymentList` (loan detail), the members tab ("loan ZMW 50, repaid ZMW 13.75") and the desktop loan pages ("Repaid so far", "Still owed", repayments section). Unit tests in `test/loan_progress_test.dart`.
- **Names, not ids.** Raw ids are gone from the UI (transaction detail no longer prints the id/sequence; borrower/approver/recorder resolve to member names, "Unknown member" when unknown). The Sync screen keeps the node's peer address as technical info.
- **Notification taps open the record.** `SyncChange.recordId` rides in the notification payload (`NotificationTarget`); `NotificationService.taps` surfaces taps (including the cold-start one) and `AppBootstrap.onNotificationTap` routes to the group, then the transaction, loan or meeting detail.
- Sync log: transient conditions (`No inventory`, dial failures, direct-fetch misses) are `SyncEventType.warning` (amber); red is reserved for rejected records and real errors. Default contribution frequency is monthly.
- Launcher icons everywhere: Android adaptive/monochrome, iOS, macOS and Windows via `flutter_launcher_icons`; Linux sets the GTK window icon from the bundled asset (`linux/runner/my_application.cc`) and ships `linux/vbank.desktop` + `tool/brand/install-linux.sh`.
- `.github/workflows/release.yml` builds Linux (tar.gz with .desktop + icon), Windows (zip) and macOS (dmg) on a `v*` tag and attaches them to a GitHub release. No mobile releases yet. Windows also gets an **Inno Setup installer** (`windows/installer/vbank.iss`: per-user install by default, Start-menu/desktop shortcuts, registers the `vbank://` URL scheme so invite links open the app; ISCC ships on the runner). App version bumped to 1.7.0+7 for the first tagged release.
- Website (`docs/`): warmer design — Inter/Manrope, amber accent on the zinc base, hero "ticket" card, stats strip, "who it is for", phone-framed screenshot gallery, FAQ accordion, CTA band, scroll-reveal — same content, with the new icon as brand mark and favicon.
- Later the same day: the big card (`HeroCard`) is inverted only in the light theme; in the dark theme it is a lifted `scheme.card` surface with a hairline, because a white slab was too loud on a black page. Group pages use shadcn's underline `TabList` (`PageTabs`, horizontally scrollable, one full-width hairline) instead of chip tabs; chips remain for filters. The transaction detail card names the group and, when the record belongs to a loan (`loanId`), shows the loan underneath — borrowed / repaid / still owed with the progress bar — and taps through to the loan.
- **Invite before account.** Tapping an invite link on a phone with no account used to dead-end in "Please create an account first". The join screen now parks the link (`PendingInvite`, `lib/core/deeplink/pending_invite.dart`), sends the person to Create Account (which shows "You have been invited to a group…"), and both account creation and backup restore call `continuePendingInvite` to resume the join on top of Home.
- Filters and form choices (`Segmented`) are shadcn's boxed `Tabs` segmented control (scrolls horizontally past four options); page tabs stay underlined — two clearly different controls. The group header keeps one trailing "⋮" button that opens a bottom sheet with Invite members / Reports / Group settings.
- **Names everywhere, editable.** Group reports printed the peer id as the member name (`ReportService` now resolves names from the member records; departed members read "Former member"). Settings → profile opens a sheet to change the display name: `AuthNotifier.updateDisplayName` rewrites the identity row and `GroupService.renameMember` renames the member record in every group (it syncs with the next snapshot like any membership edit). The peer id is no longer shown on the profile card.
- **Report export.** `ReportExportService` (`lib/services/report_export_service.dart`) renders the report as a PDF (`pdf` package: summary + member statements, a faint rotated "vBank" watermark on every page, brand header/footer) or as an Excel workbook (a hand-written OOXML package over `archive`, because the `excel` package pins an `archive` that clashes with the IPFS stack; two sheets, bold headers, money number format, "vBank" printed page header/footer). Phones get the share sheet, desktops save to Downloads and open the file. `test/report_export_test.dart` checks both outputs. Only the mobile shell has a reports page today.
- **Sync: pushes no longer wait behind dead peers.** Seen on the desktop after a dozen test groups: every round dialled each stale member of every group (8 s per dead address, then a 10 s DHT `findProviders` timeout per group), a round ran for minutes, and a contribution recorded meanwhile was not pushed because `startManualSync` returned early while a round was busy — the phone saw it two minutes later. Now: (1) `startManualSync` during a round calls `publishLocalChanges()` immediately (a serialised publish chain: snapshots + pending transactions) and flags one re-run; (2) peers are backed off *per peer id* (15 s doubling to 5 min) after a failed request or dial, skipped in `_pullInventory`/`_dialKnownPeers` for every group, and forgiven the moment anything is heard from them (`_peerReachable`); (3) the DHT provider lookup is gone — there are no bootstrap peers, it could only time out.
- **Joining while the inviter is offline.** Invite links now carry up to four member addresses (`PeerBook.mergeForInvite`: ours first, then members we have met, one per transport — the cap keeps the QR at ~v16 since every `/` is percent-encoded), so any online member can serve the snapshot; `joinGroup` waits for *any* of them (`_awaitAnyConnected`) and takes the inviter id from the link instead of the first address. When nobody answers the join is **parked**: `PendingJoin` (`lib/core/ipfs/pending_join.dart`, settings key `pendingJoins`) keeps the link fields, the passphrase-derived key (so PBKDF2 is not repeated and the passphrase itself is never stored; it lives in the record rather than `group_keys` so the node does not advertise an inventory for a group it is not in — and it is therefore not part of backups) and the `Member` we will join as. `retryPendingJoinsNow()` runs after every sync round, outside the 60 s round budget: dial, wait 3 s for any member, fetch from connected peers only, `_completeJoin`; nobody home → per-peer backoff grows; wrong passphrase / expired or invalid invite → `permanent` with the reason. The Groups list on every shell shows a "Joining a group…" row (retry / change passphrase / cancel); success emits a `group` SyncChange, which refreshes the list and posts a notification. `_joiningGroups` stops the interactive and background paths from importing twice.
- **Internet connectivity: the relay node.** Phones behind carrier NAT cannot accept connections, so members on different networks never met. `RelayNode` (`lib/core/relay/relay_node.dart`, pure Dart on top of `IpfsNodeHost`; `bin/vbank_relay.dart`; `deploy/relay/` Dockerfile + compose + README) is an always-on peer with a public address that every device dials *out* to. It answers `inventory`, serves `/vbank/fetch`, and accepts a relay-only sync op **`put`** (`{op, groupId, cid, block(base64), addrs}` — verified with `rawCidOf`, stored, pinned, recorded in `relay_ledger.json`) after which it forwards the notification to the other peers that have talked about that group id (in-memory interest map). It holds no group key. Device side: `SettingKeys.relayAddrs`; `SyncManager.relayAddresses/addRelays/removeRelay`; relays are dialled at the start of every round and included as peers in inventory pulls, announcements, block fetches and joins; `_announce` pushes the block to connected relays and `_reconcileRelays` (each round, ≤40 pushes) makes sure the relay holds everything in our ledger. Invite links carry `relay=` so joiners configure it automatically; Settings → Sync status (mobile) and the Yaru sync page manage the list. Tested in `test/relay_node_test.dart` (relay + device over loopback). Relay addresses are preferably **`/dns4/<host>/tcp/<port>/p2p/<id>`** (`--public-host` / `VBANK_RELAY_PUBLIC_HOST`): the libp2p stack dials IPs only, so `IpfsNodeHost.resolveMultiaddr` looks the name up right before every dial — the address survives an IP change and can sit behind a CNAME. `--public-port` covers external port mapping; `--identity-seed` / `VBANK_RELAY_IDENTITY_SEED` (32 bytes, base64) pins the peer id on hosts without persistent storage. **Built-in relay**: the app ships `kBuiltInRelayHosts = ['vbank.localhost.co.zm']` (`lib/core/relay/relay_directory.dart`); `RelayDirectory` resolves `_dnsaddr.<host>` TXT records over DNS-over-HTTPS (dart:io cannot query TXT) into `/dns4/…/p2p/…` addresses, cached an hour in memory and persisted (`relay.builtin.cache`) for offline starts; `relay.builtin.enabled` (default on) lets a member switch it off; invite links carry only user-added relays. The image is published to `ghcr.io/arthurkay/vbank-relay` by `.github/workflows/relay-image.yml` (Flutter base image because the app pubspec needs the Flutter SDK; `dart build cli`; runs as uid 10001).
- **Automatic cloud backup (2026-09-03).** WhatsApp's model, always encrypted: `CloudBackupService` (`lib/services/cloud_backup_service.dart`) seals `BackupService.buildFullBackup` (payload **v3**: identity, group keys, groups, members and now every record — transactions, loans, schedules, meetings, reversals, invites; v2 files still restore) with a **backup passphrase** kept in the platform keystore (`KeystoreVault`, flutter_secure_storage) and uploads it to the member's own account: Android → Google Drive `appDataFolder` (google_sign_in 7 + googleapis, `drive.appdata` scope), iOS → the app's iCloud container `iCloud.zm.co.tickethost.vbank` (icloud_storage; entitlements in `ios/Runner/Runner.entitlements`). Opportunistic schedule: `AppBootstrap` calls `runIfDue()` on start/resume (checked hourly), daily or weekly, Wi-Fi only by default (connectivity_plus), keeps the newest 3. Restore: onboarding → "Restore from Google Drive/iCloud" lists the account's backups, downloads, then the usual passphrase step. Desktop has no cloud path (file export). Android needs an OAuth client (package + signing SHA-1) in the Google Cloud project before sign-in works, iOS the iCloud container on the App ID — see `deploy/cloud-backup/README.md`. Tests: `test/cloud_backup_service_test.dart` (fake store: scheduling, Wi-Fi rule, retention, encrypted v3 round trip).
- **Application id is `zm.co.tickethost.vbank`** (2026-09-03; was `com.vbank.vbank`): Android `namespace`/`applicationId` + Kotlin package dir, iOS/macOS bundle ids, iCloud container `iCloud.zm.co.tickethost.vbank` (entitlements + `ICloudBackupStore.containerId`), Linux `APPLICATION_ID`, Windows company/copyright (TicketHost), Play link, E2E phone driver. Installing over the old id is a different app on Android — the old install keeps its data; move with a backup.
- **Store deployment (2026-09-03), copied from arthurkay/spark:** `codemagic.yaml` (pr-checks; `android-release` → Play internal draft; `ios-release` → TestFlight, automatic signing via the Developer Portal integration) triggered by `v*` tags; Gradle reads release signing from the Codemagic `CM_KEYSTORE_*` variables first, then `android/key.properties`, else the debug key with a loud warning. The GitHub release workflow gained an `android` job that decodes `KEYSTORE_BASE64` into the same `CM_*` contract and attaches a signed APK when the secrets exist. Runbook: `DEPLOYMENT.md`.
- **Known issue — 16 KB page size (Play requirement for Android 15+ targets):** Android 16 shows a compatibility warning on debuggable builds listing native libs that are not 16 KB-aligned: ML Kit's `libbarhopper_v3.so` and camera's `libimage_processing_util_jni.so` (via mobile_scanner) report *LOAD segment not aligned*; sqlcipher/sqlite3mc, libflutter/libapp report *Unknown error* (analysis, not necessarily misaligned). To clear before a Play release: AGP ≥ 8.5.1, `android.useLegacyPackaging=false`, and bump mobile_scanner / ML Kit and sqlcipher_flutter_libs to 16 KB-ready versions; verify with `zipalign -c -P 16 -v 4 app-release.apk`.
- **No more group passphrase (2026-09-03).** New groups get a random 256-bit data key (`GroupKeyService.setRandom`). An invite (`InviteService.createInvite` with the group key) now lasts **12 hours**, is single-use, and stores the group key **wrapped** under a fresh one-time secret (`InviteKeyWrap`: HKDF(secret, group, invite id) → XChaCha20-Poly1305 with AAD binding group and invite id). The secret rides in the link (`k=`, base64url); the group snapshot envelope carries, **in the clear**, `wraps: {inviteId → wrapped key}` for live invites only (`InviteService.liveInvites`), so a joiner unwraps, opens the snapshot, verifies the invite and joins — and a used or expired link unwraps nothing once admins republish. The passphrase path remains for legacy links (no `k`) and `createGroup(passphrase:)` stays optional for old data/tests. Parked joins store the secret instead of a derived key. Create-group screens lost the passphrase fields and default `requireApproval` to on; join screens prompt for a passphrase only on legacy links. Removing a member still needs key rotation (planned: versioned key ring in the same snapshot header). Schema v5 adds `invites.wrapped_key`; backups carry it.
- **Key rotation and the re-join block (2026-09-03).** Removing a member now (1) **rotates the group key**: `GroupKeyService.rotate` mints version n+1 (all versions kept in `group_key_history`; `SyncEnvelope.keyVersion`/`kv` names the version, bound into the AAD), records published afterwards use it, and the removed device's old keys open none of them; (2) **re-keys the remaining members**: every identity derives a deterministic X25519 pair from its signing seed (`MemberKeys.fromIdentitySeed`), publishes the public half once per group (`SyncPayloadType.memberKey`, signed; also carried in the join record and the roster as `Member.encKey`), and the owner's snapshot header carries the key ring sealed per active member (`rekeys: {peerId → blob}`, ECDH + HKDF + XChaCha20, AAD binds group and recipient); invite `wraps` now carry the whole ring so joiners read history; a device without the version yet throws and retries on the next pull; (3) **blocks re-joining**: `_applyMemberJoin` rejects a join from a peer with an unlifted removal (spends the invite, notifies admins), `approveMember` refuses banned peers, and the owner can **allow back** (`liftRemoval`, `MemberRemoval.lifted`, never reverts in merges). Schema v6: `members.enc_key`, `member_removals.lifted`, `group_key_history`. Backups carry the ring and enc keys. Caveat: a member whose device has not yet published its encryption key (pre-upgrade) receives the rotated key only after its next round announces it and an admin republishes.
- The app icon now replicates the welcome-page mark: a `#0B0B0C` rounded square (radius ¼) with Lucide's `landmark` glyph at half the side (`assets/brand/icon.svg`; foreground and status-bar variants alongside; `tool/brand/README.md`). All launcher icons, the Android status-bar glyph and the website icons were regenerated from it.

**v1.6 (2026-08-27) — Two-device sync that actually converges (desktop owner ↔ phone member)**
- Verified end to end with `integration_test/e2e_desktop_peer_test.dart` (Linux/Yaru owner) driving a real phone through `tool/e2e/phone.sh`: create group → invite link → phone joins → owner approves → 3 contributions → phone requests a loan → owner approves, disburses, records a repayment → schedules a meeting; every record arrives on the other device (`summary.json` written by the desktop side).
- **The IPFS node runs in a worker isolate** (`lib/core/ipfs/ipfs_node_host.dart` + `ipfs_worker.dart`; `IpfsService` is the main-isolate proxy with the same API). libp2p handshakes, DHT/bitswap and block fetches never touch the UI isolate. The worker forwards its prints to the main isolate (test harnesses only capture the root isolate) and exits when the root isolate exits (`Isolate.addOnExitListener`), so a restarted app never fights a stale node for port 4001; the host also verifies that the listener really accepts connections before reporting "started".
- **Transport is vBank's own, dart_ipfs's pubsub is not used for delivery.** dart_ipfs 1.11.7's pubsub mesh never fills and its inbound messages are never surfaced, so three libp2p protocols carry sync: `/vbank/notify/1.0.0` (push "new CID" nudges, flooded to known peers), `/vbank/fetch/1.0.0` (direct block pull, verified by recomputing the CID — `raw_cid.dart`), `/vbank/sync/1.0.0` (request/response; `inventory` returns every record CID a node holds for a group). Bitswap remains as a last resort.
- **Pull-based catch-up.** `SyncLedger` (settings key `ledger.<groupId>`) records every CID published or applied; on each sync round, after any applied notification, and on every join, a node asks each known peer for its inventory and fetches what it lacks (oldest first, capped per round). Lost notifications therefore heal within one round (2 min).
- **Addressing.** Invite links carry the inviter's TCP multiaddrs (`addrs`), snapshots carry `publisherAddrs`, joins and notifications carry the sender's `addrs`; `PeerBook` keeps them per group, replaces a stale peer id at a known transport address across all groups, and dial failures back off (15 s → 5 min) instead of forgetting. `dialableAddresses` is TCP-only (dart_ipfs cannot dial its own QUIC/WebTransport listeners). No public bootstrap peers, DHT, content routing or relay: they never helped on a LAN and their timeouts alone blew the 60 s sync budget the user saw in the Sync log.
- **Persistent identity.** A 32-byte seed beside the datastore gives the node a stable peer id across restarts; dart_ipfs declares `libp2pIdentitySeed` but never read it, so `dart_ipfs` is vendored (`third_party/dart_ipfs`, `dependency_overrides`) with four small patches documented in `third_party/dart_ipfs/VBANK_PATCHES.md`: pass the seed; await async protocol handlers before reading again (replies were written to an already-closed stream); keep dialed addresses for 24 h instead of 10 min (`No addresses found for peer`); redial once on a dead connection (`_openStreamWithRetry`).
- **Correctness fixes surfaced by the run:** signed timestamps are formatted at millisecond precision (`TransactionService.signedInstant`) — `DateTime.now()` carried microseconds the database dropped, so every synced transaction/loan failed its signature check on the receiving side; `myMembershipProvider` reads the live group (the "waiting for approval" screen never cleared); `loanProvider`/`loanScheduleProvider` watch `dataVersionProvider` (loan detail did not refresh after the owner's own approve); `DeviceSecret` prefers the fallback file (a slow Keystore on first run minted a second key and the encrypted database could not be opened again); `publishRecord` treats a failed announcement as non-fatal.
- **Phone freeze root causes:** shadcn's `TextField` defaults to `cursorOpacityAnimates: true`, a ticker that renders ~90 fps for as long as a field has focus — on a low-end phone that starved input and produced "vBank isn't responding"; every text field now passes `cursorOpacityAnimates: false` (guarded by `test/sheet_frame_loop_test.dart`). Back on the root route now sends the app to the background (`MainActivity` `moveTaskToBack`) instead of finishing the activity, which killed the sync node.
- Logging: `[sync]` lines print in debug **and profile** builds (the phone side of the E2E is a profile build; the debug build's ~580 MB footprint on a 4 GB phone was itself a source of stalls). `Libp2pRouter.debugLog` prints dial/stream failures.
- Tests: `test/ipfs_host_pair_test.dart` runs two real nodes over loopback (fetch, request/response, notify); `test/peer_book_test.dart`, `test/sync_ledger_test.dart`, `test/block_cid_test.dart`; `integration_test/linux_shell_test.dart` scopes dialog fields to the `AlertDialog`.
- UI (2026-08-27): headers never exceed two lines; transaction rows and notifications name the member and the group ("Contribution from Grace · Ngombe Circle"); Join and New group both stay on screen as floating actions; the reports page keyed its provider on a fresh microsecond timestamp every build and never finished loading; new monochrome "member ticket" app icon generated from `assets/brand/*.svg` (see `tool/brand/README.md`) with a matching status-bar glyph.
- Known limits: ipfs_libp2p keeps one process-wide "self", so in-process tests exercise B→A only; Linux packaging has no launcher icon yet; `Isolate.exit` on root-isolate exit relies on Android actually finishing the activity (Back now backgrounds instead).
