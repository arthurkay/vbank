...
class Libp2pRouter implements RouterInterface {
  /// vBank: when true, dial/stream failures are printed (the package logger is
  /// silent at the app's log level).
  static bool debugLog = false;
  static void _dbg(String m) {
    if (debugLog) print('[libp2p_router] $m');
  }
  ...
  Future<void> connect(String multiaddress) async {
    ...
    try {
      ...
      // vBank: keep dialed addresses for a day rather than ten minutes. vBank
      // peers are few and long-lived; once the entry expired every newStream
      // to a peer we were still connected to failed with "No addresses found".
      ...
      _dbg('connected to $multiaddress');
    } catch (e, stackTrace) {
      _logger.error('Failed to connect to $multiaddress', e, stackTrace);
      _dbg('connect to $multiaddress failed: $e');
      rethrow;
    }
  }
  ...
  Future<void> sendMessage(
    ...
    try {
      final stream = await _openStreamWithRetry(peerIdStr, protocol);
      ...
      _dbg('sendMessage($protocol) to $peerIdStr failed: $e');
  ...
  Future<Uint8List?> sendRequest(
    ...
    try {
      final stream = await _openStreamWithRetry(peerId, protocolId);
      ...
      try {
        ...
        if (response == null) _dbg('sendRequest($protocolId) to $peerId: no reply (EOF)');
        return response;
      } finally {
        await stream.close();
      }
      ...
      _dbg('sendRequest($protocolId) to $peerId failed: $e');
      return null;
    }
  }

  /// vBank: opens a stream to [peerIdStr]; if that fails, drops every
  /// connection to the peer and redials its last known address once. The
  /// swarm keeps a connection whose remote has silently gone away (a phone
  /// whose app restarted), and `newStream` on that dead session fails for good
  /// while `connect` keeps reporting success.
  Future<libp2p.P2PStream<dynamic>> _openStreamWithRetry(String peerIdStr, String protocol) async {
    final pid = libp2p.PeerId.fromString(peerIdStr);
    try {
      return await _host!.newStream(pid, [protocol], libp2p.Context(timeout: const Duration(seconds: 15)));
    } catch (e) {
      final addrs = _peerAddresses[peerIdStr];
      if (addrs == null || addrs.isEmpty) rethrow;
      _dbg('newStream($protocol) to $peerIdStr failed ($e); redialing ${addrs.first}');
      try {
        await _host!.network.closePeer(pid);
      } catch (_) {}
      _connectedPeers.remove(peerIdStr);
      final addr = addrs.first;
      await connect(addr.contains('/p2p/') ? addr : '$addr/p2p/$peerIdStr');
      return await _host!.newStream(pid, [protocol], libp2p.Context(timeout: const Duration(seconds: 15)));
    }
  }
  ...
  void registerProtocolHandler(
    ...
    if (_host != null) {
      _host!.setStreamHandler(protocolId, (stream, remotePeerId) async {
        ...
        _dbg('inbound stream from $remoteIdStr for $protocolId');
        ...
        try {
          ...
          while (true) {
            ...
            if (data == null) {
              _dbg('inbound $protocolId from $remoteIdStr: stream ended');
              break;
            }
            if (data.isEmpty) {
              _logger.warning(
            ...
            final packet = NetworkPacket(
              ...
              responder: (response) async {
                try {
                  ...
                  _dbg('responder write to $remoteIdStr on $protocolId failed: $e');
                }
              },
            );
            // vBank: handlers may be async and reply through `packet.responder`.
            // Wait for them, otherwise the loop below reads EOF and closes the
            // stream before the reply is written.
            final dynamic outcome = (handler as Function)(packet);
            if (outcome is Future) await outcome;
          ...
          _dbg('inbound handler error for $protocolId from $remoteIdStr: $e');
        } finally {
